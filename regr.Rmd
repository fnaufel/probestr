...
* O pacote `gglm` (http://graysonwhite.com/gglm/) tem funções para produzir gráficos úteis sobre os resíduos. Lembre-se de que a variável `modelo` guarda o objeto retornado pela função `lm`:
...
* Ou, se você não se importar com a feiúra dos gráficos do R base, pode usar a função `plot`:

    ```{r}
    plot(modelo)
    ```

* Vamos discutir apenas os dois primeiros gráficos: [resíduos pelos valores estimados]{.hl} e ???

* As observações com os maiores resíduos, $3$ delas marcadas com os números nos gráficos gerados pelo R base, são
...
* Que condições devemos verificar sobre os resíduos?
  ...
  1. Devemos confirmar que [os resíduos seguem uma distribuição normal]{.hl}.
     ...
     Pelo histograma e pelo gráfico QQ acima, os resíduos não parecem ter distribuição normal.
  ...
  2. Devemos confirmar que [os resíduos não dependem dos valores estimados]{.hl}. 
     ...
     A idéia é que o *scatter plot* dos resíduos pelos valores estimados [não deve apresentar nenhum padrão]{.hl}. A nuvem de pontos deve se distribuir de forma aparentemente aleatória em torno da reta horizontal $y = 0$ (a média dos resíduos), sem *outliers* e sem alterações significativas da variância.
     ...
     De novo, os *outliers* são o problema. Se forem ignorados, os resíduos 
...
* O *scatter plot* [não deve ter padrões visíveis:]{.hl}
...
* No *scatter plot*, [os resíduos não devem ter variabilidade diferente, dependendo do valor estimado:]{.hl}
