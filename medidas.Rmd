...
* []{#desvio-medio-zero} Não foi uma boa idéia. [O desvio médio sempre é igual a zero]{.hl}.^[Você vai provar isto em um exercício.]
...
* Para as alturas do exemplo dos jogadores de basquete:
  ...
  O CV é aproximadamente $`r statip::cv(medidas$altura) %>% fm(4)`$. 
    ...
    ```
  Em outras palavras, para as alturas, um desvio-padrão corresponde a $`r statip::cv(medidas$altura) * 100`\%$ da média.
...
* Para os pesos:
  ...
  Em outras palavras, para os pesos, um desvio-padrão corresponde a $`r statip::cv(medidas$peso) * 100`\%$ da média.
...
* Em R, a função `scale` faz isso:
  ...
  ```{r score-altura}
  medidas <- medidas %>% 
    mutate(altura_padronizada = scale(altura))
